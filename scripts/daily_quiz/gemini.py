...
def build_client() -> tuple[genai.Client, str]:
    ...
    model = (os.environ.get("GEMINI_MODEL") or "").strip() or "gemini-2.0-flash-lite"
...
def complete(client: genai.Client, model: str, system: str, user: str) -> str:
    ...
    try:
        ...
        if "404" in str(e) or "NOT_FOUND" in str(e):
            ...
            print(f"  3. You need to use a versioned model ID (e.g., gemini-2.0-flash-lite)", file=sys.stderr)
            print(f"\n  Try setting GEMINI_MODEL environment variable to one of:", file=sys.stderr)
            print(f"    - gemini-2.0-flash-lite", file=sys.stderr)
            print(f"    - gemini-2.0-flash", file=sys.stderr)
            print(f"    - gemini-2.0-flash-001", file=sys.stderr)
            print(f"\n  Example:", file=sys.stderr)
            print(f"    export GEMINI_MODEL=gemini-2.0-flash-lite", file=sys.stderr)
